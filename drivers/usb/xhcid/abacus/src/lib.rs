...
fn add_imports() -> proc_macro2::TokenStream {
    quote!(
        use power_manager::{
            Peripheral, State, SubState, StateEnum, Reg, Store, PowerError, Merge,
            AnyReg, SyncState, AnySubState, ConcreteSubState, MergeSubState, StaticRef
        };
...
pub fn process_register_block(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let register_block_str = format!("{}{}", parsed_input.peripheral_name, "Registers");
...
// Copyright Tock Contributors 2022.
    ...
    // let block = quote! {
    //     pub struct #register<S: power_manager::State> {
    //         reg:  StaticRef<#register_block<S>>,
    //     }

    //      impl <S: State> #register<S> {
    //          pub unsafe fn new(base_addr: usize) -> #register<S> {
    ...
    let struct_name_ident = format_ident!("{}Registers", parsed_input.peripheral_name);
        ...
        impl <A: Reg, B: Reg> RegisterResult<A, B> {
                ...
                D: StateEnum + From<B>,
            {
                match self {
                    ...
                    RegisterResult::Err(PowerError(reg)) => Err(PowerError(reg.into()))
                }
            }
        }
